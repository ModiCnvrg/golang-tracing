...
func (p *Producer) Produce(ctx context.Context, msg *kafka.Message, deliveryChan chan kafka.Event) error {
	s := p.startSpan(ctx, internal.OperationProduce, msg)
...
func (p *Producer) ProduceChannel(ctx context.Context) chan *kafka.Message {
...
func (p *Producer) startSpan(ctx context.Context, operationName internal.Operation, msg *kafka.Message) oteltrace.Span {
	opts := []oteltrace.SpanStartOption{
		oteltrace.WithSpanKind(oteltrace.SpanKindProducer),
	}

	carrier := NewMessageCarrier(msg)
	ctx_ := p.propagator.Extract(ctx, carrier)

	ctx_, span := p.tracer.Start(ctx_, string(operationName), opts...)

	p.propagator.Inject(ctx_, carrier)
